...
class L0L2Constrained_LogReg:
    def __init__(self, data_train, data_val=None, W_init=None):
    ...
    def estimate_W(
        ...
        for pred_idx in range(self.P):
            ...
            for alpha in alpha_list:
                beta = train_L0L2_LogReg(
                    X_train, y_train, alpha, K, beta_start=beta_start
                )

                if estimate_on_support:
                    support = np.where(beta != 0)[0]
                    beta_support = train_L0L2_LogReg(
...
def train_L0L2_LogReg(X, y, alpha, K, eta=1e-3, T_max=300, beta_start=None):
    N, P = X.shape
    old_beta = np.ones(P)
